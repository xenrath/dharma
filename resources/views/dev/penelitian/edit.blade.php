    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <a href="{{ url('dev/penelitian') }}" class="btn btn-secondary btn-flat float-left mr-2">
        ...
        <section class="content">
            <div class="container-fluid">
                <form action="{{ url('dev/penelitian/' . $penelitian->id) }}" method="POST" enctype="multipart/form-data"
                    autocomplete="off">
                    @csrf
                    @method('PUT')
                    <div class="card rounded-0">
                        <div class="card-header">
                            <h3 class="card-title">Form Penelitian</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group mb-2">
                                        ...
                                        <input type="text" class="form-control rounded-0"
                                            value="{{ $penelitian->user->nama }}" readonly>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group mb-2">
                                        ...
                                        <input type="number"
                                            ...
                                            id="tahun" name="tahun"
                                            value="{{ old('tahun', $penelitian->tahun, $penelitian->tahun) }}">
                                        @error('tahun')
                            ...
                            <div class="form-group mb-2">
                                ...
                                <textarea class="form-control rounded-0 @error('judul') is-invalid @enderror" name="judul" id="judul"
                                    cols="30" rows="4">{{ old('judul', $penelitian->judul) }}</textarea>
                                @error('judul')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                        ...
                                        <select
                                            ...
                                            @foreach ($jenis_penelitians as $jenis_penelitian)
                                                <option value="{{ $jenis_penelitian->id }}"
                                                    {{ old('jenis_penelitian_id', $penelitian->jenis_penelitian_id) == $jenis_penelitian->id ? 'selected' : '' }}>
                                        ...
                                        <select
                                            ...
                                            @foreach ($jenis_pendanaans as $jenis_pendanaan)
                                                <option value="{{ $jenis_pendanaan->id }}"
                                                    {{ old('jenis_pendanaan_id', $penelitian->jenis_pendanaan_id) == $jenis_pendanaan->id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group mb-2">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number"
                                                ...
                                                value="{{ old('dana_setuju', $penelitian->dana_setuju) }}">
                                        </div>
                                        @error('dana_setuju')
                            ...
                            <div class="form-group mb-2">
                                ...
                                <select class="custom-select rounded-0 @error('status') is-invalid @enderror" name="status"
                                    id="status">
                                    <option value="">- Pilih -</option>
                                    <option value="menunggu" {{ old('status', $penelitian->status) == 'menunggu' ? 'selected' : '' }}>
                                        Menunggu
                                    </option>
                                    <option value="revisi" {{ old('status', $penelitian->status) == 'revisi' ? 'selected' : '' }}>
                                        Revisi
                                    </option>
                                    <option value="selesai" {{ old('status', $penelitian->status) == 'selesai' ? 'selected' : '' }}>
                                        Selesai
                                    </option>
                                </select>
                                @error('status')
                    ...
                    <div class="text-right pb-4">
                        <button type="submit" class="btn btn-primary btn-flat">Perbarui Penelitian</button>
                    </div>
                </form>
            </div>
        </section>
...
@section('script')
    <script>
        ...
        var personels = @json(old('personels', $penelitian->personels->pluck('user_id')));
        if (personels !== null) {
        ...
        // 
        var old_mahasiswa = @json(session('old_mahasiswas') ?? $penelitian->mahasiswas);
