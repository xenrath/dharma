    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <a href="{{ url('operator/proposal-laporan') }}" class="btn btn-secondary btn-flat float-left mr-2">
        ...
        <section class="content">
            <div class="container-fluid">
                <form action="{{ url('operator/proposal-laporan/' . $laporan->id) }}" method="POST" autocomplete="off">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control rounded-0 @error('nomor') is-invalid @enderror"
                                    id="nomor" name="nomor" value="{{ old('nomor', $laporan->nomor) }}">
                                @error('nomor')
                            ...
                            <div class="form-group mb-2">
                                ...
                                <textarea class="form-control rounded-0 @error('perihal') is-invalid @enderror" name="perihal" id="perihal"
                                    cols="30" rows="4">{{ old('perihal', $laporan->perihal) }}</textarea>
                                @error('perihal')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                    ...
                                    <select class="select2" id="kepadas" name="kepadas[]" multiple="multiple"
                                        ...
                                        @foreach ($fakultases as $fakultas)
                                            <option value="{{ $fakultas->id }}"
                                                {{ in_array($fakultas->id, old('kepadas', $laporan->kepadas) ?? []) ? 'selected' : '' }}>
...
@section('script')
    ...
    <script>
        ...
        var proposal_ids = @json(old('proposal_ids', $laporan_proposals->pluck('id')));
