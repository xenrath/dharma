...
class ProposalPengabdianController extends Controller
{
    ...
    {
        $pengabdians = ProposalPengabdian::select(
            'id',
            'user_id',
            'tahun',
            'judul',
        ...
        return view('operator.proposal.pengabdian.index', compact('pengabdians'));
    }
    ...
    public function create()
    {
        ...
        return view('operator.proposal.pengabdian.create', compact('jenis_pendanaans', 'dosens'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $berkas = 'proposal/pengabdian_' . $waktu . '.' . $request->berkas->getClientOriginalExtension();
        ...
        return redirect('operator/proposal-pengabdian');
    }

    public function edit($id)
    {
        if (ProposalPengabdian::where('id', $id)->value('status') != 'menunggu') {
            return view('error.500');
        }

        $pengabdian = ProposalPengabdian::where('id', $id)
            ->select(
                'id',
                'user_id',
                'tahun',
        ...
        return view('operator.proposal.pengabdian.edit', compact('pengabdian', 'jenis_pendanaans', 'dosens'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->berkas) {
            Storage::disk('local')->delete('public/uploads/' . ProposalPengabdian::where('id', $id)->value('berkas'));
            $waktu = Carbon::now()->format('ymdhis');
            $berkas = 'proposal/pengabdian_' . $waktu . '.' . $request->berkas->getClientOriginalExtension();
            $request->berkas->storeAs('public/uploads/', $berkas);
        } else {
            $berkas = ProposalPengabdian::where('id', $id)->value('berkas');
        }

        $proposal = ProposalPengabdian::where('id', $id)->update([
            'user_id' => $request->ketua_id,
            'tahun' => $request->tahun,
            'judul' => $request->judul,
            'jenis_pendanaan_id' => $request->jenis_pendanaan_id,
    ...
    {
        $berkas = ProposalPengabdian::where('id', $id)->value('berkas');
        $proposal = ProposalPengabdian::where('id', $id)->delete();
